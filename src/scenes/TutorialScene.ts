...
export default class TutorialScene extends Phaser.Scene {
    ...
    private createTutorialUI(): void {
        ...
        this.nextButton.on('pointerover', () => this.nextButton.setStyle({ color: '#ffff00' }));
        this.nextButton.on('pointerout', () => this.nextButton.setStyle({ color: '#ffffff' }));
    ...
    private createControlsPanel(): void {
        ...
        for (const binding of bindings) {
            const keyText = this.add.text(-180, yPos, binding.key, { fontFamily: 'Arial', fontSize: '16px', color: '#ffff00' });
            keyText.setOrigin(0, 0.5);
            this.controlsPanel.add(keyText);

            const actionText = this.add.text(0, yPos, binding.action, { fontFamily: 'Arial', fontSize: '16px', color: '#ffffff' });
    ...
    private updateTutorialText(): void {
        if (this.tutorialTexts.length > 0) this.tutorialTexts[0].setText(this.tutorialSteps[this.currentStep]);
    }
